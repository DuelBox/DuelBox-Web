...
|---|---|---|
| **Shell** | `shellBytes` — **130.1 KB** (133,230 B) | Everything a visitor downloads before choosing a game: the scripts every non-play route loads eagerly. Does not grow when a game is added, so it is the number worth defending. Re-derived for the i18n framework (#219) at measured plus the 856 B of headroom it had before — `size-budget.json` → `_rederived_2026_09_12_i18n`. |
| **One locale** | `localeBytes` — **4.0 KB** (4,096 B) | The largest catalogue chunk: the one language a player chose, downloaded when they choose it and never before (#219). English has none — the English string is the key. Set for today's two pseudo-locales, not for the site translated; #220 re-derives it from a full extraction (`docs/i18n.md`). |
