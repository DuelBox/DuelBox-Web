...
|---|---|---|
| **Shell** | `shellBytes` — **132.5 KB** (135,630 B) | Everything a visitor downloads before choosing a game: the scripts every non-play route loads eagerly. Does not grow when a game is added, so it is the number worth defending. Raised for #220's prose pages at measured plus the same 856 B of headroom — `size-budget.json` → `_raised_2026_09_12_i18n_prose_shell`, which is also where to read why a `<T>` in a server component is not free: webpack inlines the whole of `T.tsx` into each route chunk that uses it, and this line is the union of every non-play route's chunks. |
| **One locale** | `localeBytes` — **48.0 KB** (49,152 B) | The largest catalogue chunk: the one language a player chose, downloaded when they choose it and never before (#219). English has none — the English string is the key. Raised for #220's first extraction — 445 strings, the 108 game rules and their control descriptions among them — and **still not the final number**: the other territories add their own, and one full extraction re-derives it (`size-budget.json` → `_raised_2026_09_12_i18n_prose_locale`). |
