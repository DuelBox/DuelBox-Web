...
|---|---|---|
| **Shell** | `shellBytes` — **132.5 KB** (135,723 B) | Everything a visitor downloads before choosing a game: the scripts every non-play route loads eagerly. Does not grow when a game is added, so it is the number worth defending. Re-derived for the i18n framework (#219), then raised for #220's first extraction territory — measured 134,867 B plus the same 856 B of headroom both re-derivations kept. The 2,493 B is webpack chunking, not the copy: `<T>` renders its English id unchanged, and a client component referenced from several server routes is emitted into each route's chunk. `size-budget.json` → `_raised_2026_09_12_i18n_catalogue`. |
| **One locale** | `localeBytes` — **16.0 KB** (16,384 B) | The largest catalogue chunk: the one language a player chose, downloaded when they choose it and never before (#219). English has none — the English string is the key. Measured 14,256 B for `en-XA` and 11,587 B for `ar-XB` over 185 msgids, which is **one** of #220's six extraction territories — a waypoint that lets this branch build, not the number for the site translated. The integrator re-derives it from the complete extraction (`docs/i18n.md`). |
