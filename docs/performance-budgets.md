...
|---|---|---|
| **Shell** | `shellBytes` — **131.1 KB** (134,207 B) | Everything a visitor downloads before choosing a game: the scripts every non-play route loads eagerly. Does not grow when a game is added, so it is the number worth defending. Re-derived once for #220's whole extraction at measured plus the same 856 B of headroom the framework kept — `size-budget.json` → `_rederived_2026_09_12_extraction`, which says what the bytes bought (the `t()` and `<T>` wrappers, not the strings: the data strings are in the locale chunk) and why the five territories' provisional raises were not summed: the union measures 1.4 KB *less* than the largest of them did alone, because webpack's split settled once every route imported the i18n modules. |
| **One locale** | `localeBytes` — **82.0 KB** (83,968 B) | The largest catalogue chunk: the one language a player chose, downloaded when they choose it and never before (#219). English has none — the English string is the key. Re-derived for #220's full extraction — 884 strings, the pseudo-locale `en-XA` at 67,125 B measured, plus about 25% (`size-budget.json` → `_rederived_2026_09_12_extraction`). Held at that number for #223, which padded `en-XA` to 150% of the source length and moved it to 68,375 B measured at integration, 886 strings (68,170 B on its own branch) — half again the characters for under 2% more bytes, because the padding is a run of one repeated dot and gzip prices a run at nothing (`_measured_2026_09_12_expansion`). Held again at the seat swap (#161), which added the swap control's two strings: 888 strings, `en-XA` 68,543 B measured, 15,425 B of room left. A pseudo-locale is this bucket's worst case by design, since `en-XA` doubles every ASCII byte and now carries the largest expansion the layout is asked to survive, so a reviewed translation (#221) of the same keys is expected to land under it. |
...
| **On demand** | `onDemandBytes` — **63.0 KB** (64,512 B) | The play route's own scripts plus anything reached by an `import()`. Nobody downloads it by arriving; it is paid when a player commits to a game. |
...
| **First session** | `firstSessionBytes` — **338.0 KB** (346,112 B) | Arrive, pick a game, play it: the landing document, its stylesheets, the faces an English page actually fetches (the range rule below, not the filename), the shell, the worker, the play route's code and the largest game chunk. Wire bytes, derived from the lines above plus the three non-JavaScript ones nothing weighed before (#2446). Raised to 335,872 B at the integration of #220 and #223; #196's larger worker brings the final combined measurement to 335,928 B. The derived line retains more slack than its budgeted parts together (`size-budget.json` → `_raised_2026_09_12_download_all`). |
| **Browsing session** | `browsingSessionBytes` — **784.0 KB** (802,816 B) | Arrive at the catalogue and scroll to the end of it, pressing nothing: the catalogue document, stylesheets, faces, shell, worker and all 108 speculated payloads. The most a visitor who buys nothing can cost (#2446). With #196 and the current main, measured 798,585 B; the line retains more slack than its budgeted parts together. |
