...
describe('what the pause panel says about a controller', () => {
  it('names whose seat a new pad will drive', () => {
    expect(
      gamepadNotice({}, { kind: 'connected', seat: 'p2', gamepadIndex: 1, id: 'x' }, names),
    ).toBe("A controller was plugged in. It will drive Kim's seat.");
  });

  it('says a pad with no free seat drives nobody, and how to change that', () => {
    const text = gamepadNotice(
      {},
      { kind: 'connected', seat: null, gamepadIndex: 2, id: 'x' },
      names,
    );
  ...
  it('tells the player whose pad went dead that their seat still works', () => {
    const text = gamepadNotice(
      {},
      { kind: 'disconnected', seat: 'p1', gamepadIndex: 0, id: 'x' },
      names,
    );
