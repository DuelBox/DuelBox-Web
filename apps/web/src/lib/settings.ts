 ...
 * gameSpeed, locale }`. The version is 1 and stays 1 through the three fields added for
 * the accessibility wave and the locale added after them: each reads back as its default
 * when absent, so no migration is owed and a blob from before they existed is still a
 * valid version-1 blob.
 */
const VERSION = 1;
