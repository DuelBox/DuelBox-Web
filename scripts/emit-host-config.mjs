...
/**
 * Netlify, Cloudflare Pages, and Cloudflare Workers static assets — which reads this file
 * too, and has since April 2025, so `wrangler.jsonc` needs nothing adding to it.
 *
 * Three rules: the security set on every path, and the two disjoint cache rules (#188).
 * Well inside Cloudflare's 100, and disjointness is not a tidiness preference — an overlap
 * is *joined* rather than resolved on that host, so two rules setting `Cache-Control` for
 * one path produce one useless header. `scripts/cache-headers.mjs` has the quote and the
 * arithmetic; `check-headers.mjs` fails the build if two rules ever match one file.
 */
function renderHeadersFile(entries) {
  ...
  for (const rule of CACHE_RULES) {
    lines.push('', `# ${rule.why}`, rule.path, `  Cache-Control: ${rule.value}`);
  }
  lines.push(
    '',
    '# Everything else — documents, index.txt payloads, the manifest, .well-known — is',
    '# deliberately unlisted: a catch-all here would be joined with the rule above rather',
    '# than overridden by it. Cloudflare and Netlify both default those to',
    `# "${REVALIDATE_CACHE_CONTROL}", which is the value this would have written.`,
  );
  return lines.join('\n') + '\n';
}
 ...
 *
 * The one host config that can state the document rule outright, because `source` is a
 * path-to-regexp pattern and a complement is expressible. The three cache sources are
 * mutually exclusive, so nothing here depends on a precedence rule Vercel does not document.
 */
    ...
    JSON.stringify(
      {
        ...
        headers: [
          { source: '/(.*)', headers: entries.map(([key, value]) => ({ key, value })) },
          ...VERCEL_CACHE_RULES.map((rule) => ({
            source: rule.source,
 ...
 *
 * The cache rules are the one place these three are *better off* than `_headers`: an nginx
 * `location`, an Apache `<If>`/`<Else>` and a Caddy matcher are each exclusive, so the
 * default and its exception can be written side by side without either joining onto the
 * other. What nginx gives back with the other hand is `add_header`: a location block that
 * sets one discards every header inherited from the server block, which is how a security
 * set goes missing on exactly the files nobody thinks to scan. Said out loud below.
 */
...
${nginx
  ...
  .map((line) => `#${line}`)
  .join('\n')}
#   add_header Cache-Control "${REVALIDATE_CACHE_CONTROL}" always;
#
#   # A location that sets any add_header discards the ones inherited from here, so put the
#   # block above in an include and pull it into both of these. Dropping it is silent.
#   location ^~ ${HASHED_ASSET_PREFIX} {
#     include /etc/nginx/duelbox-headers.conf;
#     add_header Cache-Control "${IMMUTABLE_CACHE_CONTROL}" always;
#   }
#   location = ${SERVICE_WORKER_PATH} {
#     include /etc/nginx/duelbox-headers.conf;
#     add_header Cache-Control "${REVALIDATE_CACHE_CONTROL}" always;
#   }
# }
...
${apache
  ...
  .map((line) => `#${line}`)
  .join('\n')}
#   <If "%{REQUEST_URI} =~ m#^${HASHED_ASSET_PREFIX}#">
#     Header always set Cache-Control "${IMMUTABLE_CACHE_CONTROL}"
#   </If>
#   <Else>
#     Header always set Cache-Control "${REVALIDATE_CACHE_CONTROL}"
#   </Else>
# </IfModule>
...
${caddy
  ...
  .join('\n')}
#   }
#   @immutable path ${HASHED_ASSET_PREFIX}*
#   header @immutable Cache-Control "${IMMUTABLE_CACHE_CONTROL}"
#   @revalidate not path ${HASHED_ASSET_PREFIX}*
#   header @revalidate Cache-Control "${REVALIDATE_CACHE_CONTROL}"
# }
`;
}
...
async function main() {
  ...
  console.log(
    `emit-host-config: ${String(CACHE_RULES.length)} cache rule(s) in _headers and ` +
      `${String(VERCEL_CACHE_RULES.length)} in vercel.json, immutable only under ` +
      `${HASHED_ASSET_PREFIX} — all disjoint, because an overlap is joined and not resolved`,
  );
