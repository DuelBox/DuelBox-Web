...
// for, and because a missing file must fail rather than score zero. It is not one of the
// manifest-classified chunks, so it is excluded from the unclassified check below as well.
const workerFile = join(OUT, 'sw.js');
const workerBytes = files.includes(workerFile) ? (sizes.get(workerFile) ?? 0) : null;

// The shell/on-demand split is measured from the build manifests (#2516): the shell is what
// a route loads eagerly with a <script> tag, on-demand is everything reached through an
// `import()`. Both naturally exclude `sw.js`, which no route's manifest lists.
