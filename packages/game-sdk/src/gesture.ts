 ...
 * taken away rather than let go (a `pointercancel`, palm rejection, an incoming call, or the
 * shell pausing and calling `InputManager.clear()`). The engine tells a cancel from a release
 * (#2480) and suppresses the release on a cancel, so a game commits nothing at the moment its
 * gesture is disowned. The `!actionHeld` half is load-bearing: the engine raises
 * `pointerCancelled` for ANY cancelled pointer, so while a second finger is still down (or the
 * action key is still held) the gesture has not ended and its charge must not be dropped.
 * A game holding a charge across frames reads this and discards the charge, not the aim.
